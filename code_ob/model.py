...
class ImageCaptionModel(tf.keras.Model):
    ...
    def train(self, train_captions, train_image_features, padding_index, batch_size=30):
        ...
        :return:               avg loss, avg accuracy, avg perplexity
        """
        ...
        shuffled_indices = tf.random.shuffle(tf.range(0, len(train_captions), 1))
        ...
        for _, end in enumerate(range(batch_size, len(train_captions)+1, batch_size)):
            ...
            ## Perform a training pass. Make sure to factor out irrelevant labels.
...
def accuracy_function(prbs, labels, mask):
    ...
    """
    correct_classes = tf.argmax(prbs, axis=-1, output_type=tf.int32) == labels
